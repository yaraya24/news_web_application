...
class NPRSpider(Spider):
    ...
    def npr_article(self, response):
        ...
        try:
            item["image_source"] = response.xpath(
                '//div[@class="bucketwrap image large"]/div[1]/img/@src'
            ).extract()[0]
        except:
            item[
                "image_source"
            ] = "https://play-lh.googleusercontent.com/iV4u7kqNEyTbMI2Di7MXR5VyP2wZelmW2JcCqYQwwt0L4uP7AKq37T5FQ2O0XUXxXi4"
        item["author"] = (
            response.xpath('string(//p[@class="byline__name byline__name--block"])')
            .get()
            .strip()
        )
